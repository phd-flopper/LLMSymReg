...
from trainers.no_planner import CMTrainer
from trainers.no_critic import PMTrainer
from trainers.no_mutator import CPTrainer
import sympy

def run_ablation(run_name, ablation_type):

    run_path = os.path.join(config['ablation'][f'{ablation_type}_run_dir'], run_name)
    ...
    for size in tqdm(sample_sizes):
        ...
        idx = 1
        n_dicta = {'seed':int(time.time()), 'lib':lib, 'n_equations':12, 'subset':None}
        ...
        for x, y, eq in tqdm(n_train_loader):
            ...
            y = npy
            if ablation_type == 'cp':
                trainer = CPTrainer(config, X, y, eq, f'{size}_{eq}', 
                                    dom=config['experiments']['dom'][idx - 1], size=size)
            elif ablation_type == 'cm':
                trainer = CMTrainer(config, X, y, eq, f'{size}_{eq}', 
                                    dom=config['experiments']['dom'][idx - 1], size=size)
            elif ablation_type == 'pm':
                trainer = PMTrainer(config, X, y, eq, f'{size}_{eq}', 
                                    dom=config['experiments']['dom'][idx - 1], size=size)                
            trainer.run()
...
def plot_(run_dir):
    ...
    plt.tight_layout()
    plt.savefig(run_dir + '\scores.svg')
    plt.savefig(run_dir + '\scores')
    plt.show()

def plot_out_(run_dir):
    ...
    plt.tight_layout()
    plt.savefig(run_dir + '\out_scores.svg')
    plt.savefig(run_dir + '\out_scores')
    plt.show()

if __name__ == "__main__":
    config = OmegaConf.load(PATH)
    num_runs = config['ablation']['num_runs']
    if config['ablation']['cm'] == True: 
        ...
        for i in range(1, num_runs + 1):
            run_ablation(f'Trial_Nguyen_{i}', 'cm')
        
        with open(config['ablation']['cm_run_dir'] + '\scores.txt', 'w+') as f:
            for line in global_scores:
                f.write(str(line) + '\n')
            f.close()
    
        np.save(config['ablation']['cm_run_dir'] + '\global_scores.npy', global_scores)  
    
        with open(config['ablation']['cm_run_dir'] + '\out_scores.txt', 'w+') as f:
            for line in out_global_scores:
                f.write(str(line) + '\n')
            f.close()
            
        np.save(config['ablation']['cm_run_dir'] + '\out_global_scores.npy', out_global_scores)
        
        f_path = config['ablation']['cm_run_dir'] + '\equations'
            ...
            for i in range(1, 13):
                ...
                with open(f_path + f'\{config["baseline"]["sample_sizes"][j]}_{eq}.txt', 'w+') as f:
                    for line in equations[j][eq]:
                        f.write(str(line) + '\n')
            f.close()
        plot_(config['ablation']['cm_run_dir'])
        plot_out_(config['ablation']['cm_run_dir'])
    if config['ablation']['cp'] == True: 
        global_scores = [[0] * 12 for _ in range(len(config['baseline']['sample_sizes']))]
        out_global_scores = [[0] * 12 for _ in range(len(config['baseline']['sample_sizes']))]
        ...
        for i in range(1, num_runs + 1):
            run_ablation(f'Trial_Nguyen_{i}', 'cp')
        
        with open(config['ablation']['cp_run_dir'] + '\scores.txt', 'w+') as f:
            for line in global_scores:
                f.write(str(line) + '\n')
            f.close()
    
        np.save(config['ablation']['cp_run_dir'] + '\global_scores.npy', global_scores)  
    
        with open(config['ablation']['cp_run_dir'] + '\out_scores.txt', 'w+') as f:
            for line in out_global_scores:
                f.write(str(line) + '\n')
            f.close()
            
        np.save(config['ablation']['cp_run_dir'] + '\out_global_scores.npy', out_global_scores)
        
        f_path = config['ablation']['cp_run_dir'] + '\equations'
        os.makedirs(f_path, exist_ok=True)
            ...
            for i in range(1, 13):
                ...
                with open(f_path + f'\{config["baseline"]["sample_sizes"][j]}_{eq}.txt', 'w+') as f:
                    for line in equations[j][eq]:
                        f.write(str(line) + '\n')
            f.close()
        plot_(config['ablation']['cp_run_dir'])
        plot_out_(config['ablation']['cp_run_dir'])
    if config['ablation']['pm'] == True: 
        global_scores = [[0] * 12 for _ in range(len(config['baseline']['sample_sizes']))]
        out_global_scores = [[0] * 12 for _ in range(len(config['baseline']['sample_sizes']))]
        ...
        for i in range(1, num_runs + 1):
            run_ablation(f'Trial_Nguyen_{i}', 'pm')
        
        with open(config['ablation']['pm_run_dir'] + '\scores.txt', 'w+') as f:
            for line in global_scores:
                f.write(str(line) + '\n')
            f.close()
    
        np.save(config['ablation']['pm_run_dir'] + '\global_scores.npy', global_scores)  
    
        with open(config['ablation']['pm_run_dir'] + '\out_scores.txt', 'w+') as f:
            for line in out_global_scores:
                f.write(str(line) + '\n')
            f.close()
            
        np.save(config['ablation']['pm_run_dir'] + '\out_global_scores.npy', out_global_scores)
        
        f_path = config['ablation']['pm_run_dir'] + '\equations'
        os.makedirs(f_path, exist_ok=True)
            ...
            for i in range(1, 13):
                ...
                with open(f_path + f'\{config["baseline"]["sample_sizes"][j]}_{eq}.txt', 'w+') as f:
                    for line in equations[j][eq]:
                        f.write(str(line) + '\n')
            f.close()
        plot_(config['ablation']['pm_run_dir'])
        plot_out_(config['ablation']['pm_run_dir'])
